...
class EnhancedDragonFightHandler {
  ...
  async performRangedAttack(dragon) {
    ...
    // حساب نقطة التصويب مع الأخذ في الاعتبار حركة التنين
    const predictedPos = this.calculateProjectileInterception(dragon)
    await this.bot.lookAt(predictedPos)
    
    // شحن القوس بالكامل للحصول على